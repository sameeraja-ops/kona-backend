...
export default class StudentController {
  ...
  public getBatchesProgressByStudentId = async (req: Request, res: Response) => {
    try {
      const batches = await courseProgress.aggregate([
        {
          $match: {
            studentId: Types.ObjectId(req["tokenId"])
          },
        ...
        },
        {
          $project: {
            title: "$courseDetails.title",          
            progress: "$batchDetails.progress", 
            imageURL: "$courseDetails.imageURL",
          },
        },
      ]);
  ...
  public deleteProfilePic = async (req: Request, res: Response) => {
    try {
      const editStudentDetails = await StudentModel.findByIdAndUpdate(
        req["tokenId"],
