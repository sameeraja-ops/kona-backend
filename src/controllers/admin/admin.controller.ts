...
export default class AdminController {
  ...
  public allCourses = async (req: Request, res: Response) => {
    try {
      const courses = await courseModel.find({}).sort({ lastUpdatedAt: -1 });
      if (courses) {
  ...
  public getBatchDetailById = async (req: Request, res: Response) => {
    try {
      const batch: any = await courseBatchModel.findById(req.params.batchId);
      if (batch) {
        let students = await courseProgress.find({ batchId: batch["_id"] });
        res.status(200).json({
          courseId: batch["courseId"],
  ...
  public updateCourseBatch = async (req: Request, res: Response) => {
    try {
      let request = {
        ...
        //     ...a
        //   })),
        progress: commonService.calculateBatchProgress(req.body),
        ...(req.body["projects"] && {
  ...
  public allUsers = async (req: Request, res: Response) => {
    try {
      const users = await studentModel.find({}).sort({ lastUpdatedAt: -1 });
      if (users) {
        res.status(200).json(users);
      }
    } catch (error) {
